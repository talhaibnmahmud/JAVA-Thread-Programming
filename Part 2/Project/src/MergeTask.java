...
public class MergeTask implements Runnable {
    ...
    private void mergeFiles(File[] parts, File outputFileName) {
        try (FileChannel outputChannel =
                new FileOutputStream(outputFileName).getChannel()) {
            for(File fileLocation : parts) {
                try (FileChannel inputChannel =
                        new FileInputStream(fileLocation).getChannel()) {
    ...
    private File creatMainFile() throws IOException {
